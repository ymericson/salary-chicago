...
# change smaller departments to "Other"
top_level1s <- head(names(sort(table(df$department), decreasing = TRUE)), 15)
df$level1 <- df$department
df$level1[!(df$level1 %in% top_level1s)] <- "OTHER"
df$level2 <- df$job_titles
df[which(df$level1 == "OTHER"),]$level2 <- df[which(df$level1 == "OTHER"),]$department
df$level3 <- NA
df[which(df$level1 == "OTHER"),]$level3 <- df[which(df$level1 == "OTHER"),]$job_titles


head(sort(table(df[which(df$department == "Police"),]$job_titles), decreasing = TRUE), 5)
df$level1[!(df$level1 %in% top_level1s)] <- "OTHER"
# head(subset(df, level1 == "OTHER"), 10)
...
chicago_treemap <- treemap(df,
                           index=c("tot_salary_level1","tot_salary_job", "level3"),
                           vSize="annual_salary",
                           vColor="level1",
                           type="index",
...
head(names(sort(table(df$department), decreasing = TRUE)), 15)
